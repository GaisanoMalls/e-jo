<div class="modal status__modal" id="editStatus{{ $status->id }}" tabindex="-1"
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content modal__content">
            ...
            <form action="{{ route('staff.manage.ticket_statuses.update', $status->id) }}" method="post"
                    ...
                    <div class="row mb-2">
                        <div class="mb-2">
                            ...
                            <input type="text" name="name" class="form-control form__field" id="name"
                                value="{{ $status->name }}" placeholder="Enter status name">
                            @error('name', 'editStatus')
                        ...
                        <div class="mb-2">
                            ...
                            <div class="d-flex align-items-center gap-2">
                                <input type="color" name="colorSelect" class="form-control-color form__field p-1"
                                    id="statusColorSelect" value="{{ $status->color }}">
                                <input type="text" name="color" class="form-control form__field"
                                    value="{{ $status->color }}" id="statusColorHexVal"
                                    placeholder="Enter status color">
                            </div>
                            @error('color', 'editStatus')
                            <span class="error__message">
                                <i class="fa-solid fa-triangle-exclamation"></i>
                                {{ $message }}
                            </span>
                ...
                <div class="modal-footer modal__footer p-0 justify-content-between border-0 gap-2">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <button type="button" class="btn m-0 btn__modal__footer btn__cancel" id="btnCloseModal"
                            ...
                            onclick="window.location.href='{{ route('staff.manage.ticket_statuses.index') }}'">Cancel</button>
                        @else
                        <button type="button" class="btn m-0 btn__modal__footer btn__cancel" id="btnCloseModal"
