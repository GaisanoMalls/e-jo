        ...
        <div class="d-flex flex-column">
            @if ($ticket->approval_status === ApprovalStatusEnum::APPROVED)
                <button type="button"
                    ...
                    data-bs-toggle="dropdown" aria-expanded="false" style="background-color: #FF8B8B !important; color: white !important">
            ...
            @else
                <button type="button"
                    class="btn btn btn-sm border-0 m-auto rounded-circle ticket__detatails__btn__close d-flex align-items-center justify-content-center"
                    data-bs-toggle="dropdown" aria-expanded="false" @disabled(!$isAllowedToApprove)>
            ...
            @else
                <small class="ticket__details__topbuttons__label" @style(['color: rgb(153, 153, 153)' => !$isAllowedToApprove])>Approval</small>
            @endif

            @if ($ticket->approval_status !== ApprovalStatusEnum::APPROVED && $ticket->approval_status !== ApprovalStatusEnum::DISAPPROVED && $isAllowedToApprove)
                <ul class="dropdown-menu dropdown-menu-end approval__dropdown__menu slideIn animate">
                    ...
                    <li>
                        <button type="button" class="btn d-flex align-items-center gap-2 w-100 dropdown__select__button button__disapprove"
