<div wire:poll.visible.7s class="row mx-0">
    <ul class="nav nav-pills nav-fill flex-nowrap ticket__tab__header">
        <li class="nav-item">
            <a href="{{ route('user.tickets.open_tickets') }}"
                class="nav-link d-flex align-items-center justify-content-center gap-3 ticket__tab__link
                {{ Route::is('user.tickets.open_tickets') ? 'active' : '' }}"
                {{ Route::is('user.tickets.open_tickets') ? 'wire:ignore.self' : '' }} wire:ignore.self>
                <span class="ticket__count__tab">{{ $openTickets->count() }}</span>
                Open
            </a>
        </li>
        <li class="nav-item">
            <a href="{{ route('user.tickets.viewed_tickets') }}"
                class="nav-link d-flex align-items-center justify-content-center gap-3 ticket__tab__link
                {{ Route::is('user.tickets.viewed_tickets') ? 'active' : '' }}"
                wire:ignore.self>
                <span class="ticket__count__tab">{{ $viewedTickets->count() }}</span>
                Viewed
            </a>
        </li>
        <li class="nav-item">
            <a href="{{ route('user.tickets.approved_tickets') }}"
                class="nav-link d-flex align-items-center justify-content-center gap-3 ticket__tab__link
                {{ Route::is('user.tickets.approved_tickets') ? 'active' : '' }}"
                wire:ignore.self>
                <span class="ticket__count__tab">{{ $approvedTickets->count() }}</span>
                Approved
            </a>
        </li>
        <li class="nav-item">
            <a href="{{ route('user.tickets.disapproved_tickets') }}"
                class="nav-link d-flex align-items-center justify-content-center gap-3 ticket__tab__link
                {{ Route::is('user.tickets.disapproved_tickets') ? 'active' : '' }}"
                wire:ignore.self>
                <span class="ticket__count__tab">
                    {{ $disapprovedTickets->count() }}
                </span>
                Disapproved
            </a>
        </li>
        <li class="nav-item">
            <a href="{{ route('user.tickets.claimed_tickets') }}"
                class="nav-link d-flex align-items-center justify-content-center gap-3 ticket__tab__link
                {{ Route::is('user.tickets.claimed_tickets') ? 'active' : '' }}"
                wire:ignore.self>
                <span class="ticket__count__tab">{{ $claimedTickets->count() }}</span>
                Claimed
            </a>
        </li>
        <li class="nav-item">
            <a href="{{ route('user.tickets.on_process_tickets') }}"
                class="nav-link d-flex align-items-center justify-content-center gap-3 ticket__tab__link
                {{ Route::is('user.tickets.on_process_tickets') ? 'active' : '' }}"
                wire:ignore.self>
                <span class="ticket__count__tab">{{ $onProcessTickets->count() }}</span>
                On Process
            </a>
        </li>
        <li class="nav-item">
            <a href="{{ route('user.tickets.closed_tickets') }}"
                class="nav-link d-flex align-items-center justify-content-center gap-3 ticket__tab__link
                {{ Route::is('user.tickets.closed_tickets') ? 'active' : '' }}"
                wire:ignore.self>
                <span class="ticket__count__tab">{{ $closedTickets->count() }}</span>
                Closed
            </a>
        </li>
    </ul>
</div>
