    ...
    <div>
        <div class="d-flex flex-column">
            @if (auth()->user()->isServiceDepartmentAdmin() && $ticket->approval_status === ApprovalStatusEnum::FOR_APPROVAL)
                <button type="button" class="btn btn-sm border-0 m-auto ticket__detatails__btn__close d-flex align-items-center justify-content-center"
            ...
            @endif
            <small
                class="ticket__details__topbuttons__label {{ auth()->user()->isServiceDepartmentAdmin() && $ticket->approval_status === ApprovalStatusEnum::FOR_APPROVAL ? 'text-muted' : '' }}">Reply</small>
        </div>
    </div>
@endif
