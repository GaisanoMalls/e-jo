    ...
    @if (!is_null($replies))
        <div wire:poll.visible>
            @if ($ticket->replies->isNotEmpty())
                    ...
                    <div id="reply-{{ $reply->id }}"
                        ...
                        <div class="d-flex flex-column qoute__reply__action__container rounded-5 shadow"
                            ...
                            @if ($this->isLiked($reply))
                                <button class="btn btn-sm d-flex align-items-center justify-content-center rounded-circle btn__reply__qoute__action"
                            ...
                            <button
                                ...
                                data-bs-toggle="modal" data-bs-target="#qouteReplyModal" wire:click="qouteReply({{ $reply->id }})">
                                <i class="bi bi-reply-fill" style="{{ $reply->user_id != auth()->user()->id ? 'transform: scaleX(-1);' : '' }}"></i>
                            </button>
                        </div>
                                ...
                                <div class="text-muted qouted__reply position-relative"
                                    ...
                                    <div class="d-flex align-items-center mb-2">
                                        @if ($reply->quotedReply->user->profile->picture)
                                            <img src="{{ Storage::url($reply->quotedReply->user->profile->picture) }}" alt=""
                                                class="image-fluid reply__qoute__user__picture me-2">
                                        @else
                                        ...
                                        @endif
                                        <small style="font-size: 0.7rem;">{{ $reply->quotedReply->user->profile->getFullName }}</small>
                                    </div>
                                    {!! Str::limit($reply->quotedReply?->description, 200, '...') !!}
                                    <a href="#reply-{{ $reply->quotedReply->id }}" class="position-absolute" style="right: 4px; top: 4px;">
                ...
                @endforeach
                <div wire:ignore.self class="modal fade modal-xl ticket__actions__modal" id="qouteReplyModal" tabindex="-1"
                    aria-labelledby="modalFormLabel" aria-hidden="true">
                            ...
                            <div class="modal__body">
                                <form wire:submit.prevent="sendQuoteReply">
                                    ...
                                    <div class="mt-4">
                                        ...
                                        <input class="form-control ticket__file__input w-auto my-3" type="file" wire:model="quoteReplyFiles"
                                    ...
                                    <button type="submit"
                                        ...
                                        <span wire:loading wire:target="sendQuoteReply" class="spinner-border spinner-border-sm" role="status"
                                            aria-hidden="true">
                                        </span>
                                        Send
