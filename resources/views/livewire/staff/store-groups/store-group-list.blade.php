<div>
    <div class="table-responsive custom__table">
        <table class="table mb-0">
            @if ($storeGroups->isNotEmpty())
                <thead>
                    <tr>
                        <th class="border-0 table__head__label" style="padding: 17px 30px;">Store Group Name</th>
                        <th class="border-0 table__head__label" style="padding: 17px 30px;">Date Created</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($storeGroups as $storeGroup)
                        <tr wire:key="store-group-{{ $storeGroup->id }}">
                            <td>
                            ...
                            <td>
                                <div class="d-flex align-items-center text-start td__content">
                                    <span>{{ $storeGroup->dateCreated() }}</span>
                                </div>
                            </td>
                            <td>
                                <div class="d-flex align-items-center justify-content-end pe-2 gap-1">
                                    <button data-tooltip="Edit" data-tooltip-position="top"
                                        ...
                                        data-bs-toggle="modal" data-bs-target="#editStoreGroupModal"
                                        wire:click="editStoreGroup({{ $storeGroup->id }})">
                                        <i class="bi bi-pencil"></i>
                                    </button>
                                    <button class="btn btn-sm action__button mt-0" data-bs-toggle="modal"
                                        data-bs-target="#deleteStoreGroupModal"
                                        wire:click="deleteStoreGroup({{ $storeGroup->id }})">
            ...
            @else
                <div class="bg-light py-3 px-4 rounded-3" style="margin: 20px 29px;">
                    <small style="font-size: 14px;">No records for store groups.</small>
                </div>
            @endif
        </table>
    </div>

    {{-- Edit Store Group Modal --}}
    <div wire:ignore.self class="modal fade department__modal" id="editStoreGroupModal" tabindex="-1"
        aria-labelledby="editStoreGroupModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content modal__content">
                <div class="modal-header modal__header p-0 border-0">
                    <h1 class="modal-title modal__title" id="editStoreGroupModalLabel">Edit Store Group</h1>
                ...
                <form wire:submit.prevent="update">
                    <div class="modal-body modal__body">
                        <div class="row mb-2">
                            <div class="mb-2">
                                <label for="name" class="form-label form__field__label">Store Group Name</label>
    ...
    {{-- Delete Store Group Modal --}}
    <div wire:ignore.self class="modal fade modal__confirm__delete__bu__department" id="deleteStoreGroupModal" tabindex="-1"
        aria-hidden="true">
            ...
            <div class="modal-content modal__content">
                <div class="modal-body border-0 text-center pt-4 pb-1">
                    ...
                    <p class="mb-1" style="font-weight: 500; font-size: 15px;">
                        Are you sure you want to delete this store group?
                    </p>
                    <strong>{{ $name }}</strong>
                </div>
                <hr>
...
@push('livewire-modal')
    <script>
        window.addEventListener('close-modal', () => {
            $('#editStoreGroupModal').modal('hide');
            $('#deleteStoreGroupModal').modal('hide');
        });

        window.addEventListener('show-edit-store-group-modal', () => {
            $('#editStoreGroupModal').modal('show');
        });

        window.addEventListener('show-delete-store-group-modal', () => {
            $('#deleteStoreGroupModal').modal('show');
        });
    </script>
@endpush
