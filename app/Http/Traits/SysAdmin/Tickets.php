...
trait Tickets
{
    ...
    {
        return Ticket::where(fn($statusQuery) => $statusQuery->where('status_id', Status::OPEN)->whereIn('approval_status', [ApprovalStatusEnum::APPROVED, ApprovalStatusEnum::FOR_APPROVAL]))
            ->orderByDesc('created_at')->get();
    }

    public function sysAdminGetClaimedTickets()
    {
        return Ticket::where(fn($statusQuery) => $statusQuery->where('status_id', Status::CLAIMED)->where('approval_status', ApprovalStatusEnum::APPROVED)->whereNotNull('agent_id'))
            ->orderByDesc('created_at')->get();
    }

    public function sysAdminGetOnProcessTickets()
    {
        return Ticket::where(fn($statusQuery) => $statusQuery->where('status_id', Status::ON_PROCESS)->whereIn('approval_status', [ApprovalStatusEnum::APPROVED, ApprovalStatusEnum::FOR_APPROVAL]))
            ->orderByDesc('created_at')->get();
    }
