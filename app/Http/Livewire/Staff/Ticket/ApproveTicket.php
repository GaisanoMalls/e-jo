                    ...
                    if ($this->ticket->status_id != Status::APPROVED &&  $this->ticket->approval_status != ApprovalStatusEnum::APPROVED) {
                        ...
                        if ($this->ticket->helpTopic->specialProject) {
                            ...
                            TicketApproval::where('ticket_id', $this->ticket->id)
                                ...
                                ->whereJsonContains('level_1_approver->approver_id', auth()->user()->id)
                                ->update([
                            ...
                            if ($filteredLevel2Approvers->isNotEmpty()) {
                                $level2Approvers = User::with('profile')->whereIn('id', $filteredLevel2Approvers->pluck('level_1_approver.approver_id')->flatten()->toArray())->get();
